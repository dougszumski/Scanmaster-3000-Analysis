        ...
        if len(data_window) == window_length:
            ...
            if groupAvg(data_window) > l_th and l_plat_len > 0 \
                ...
                if background_level > l_th:
                    lowchan_pass = False
                    print 'REJECTED: Current failed to decay below the level', l_th, 'Actual level:', \
                        background_level
                ...
                if points > controller.min_seg_len.get() and points < controller.max_seg_len.get() and sat_pass \
                    ...
                    filename = 'slice' + str(filecounter).zfill(4) \
                        + '.txt'
                    print 'Reconstructed I(s) scan of length: ', points, \
                        'as: ', filename
                    fileOutput(
                        filename,
                        data_slice,
...
root = Tk()
root.title('I(s) scan analyser v0.26')
